...
versao_app= f"w1.0 {ano_atual}"                              # Arquivo de origem - LotMaxApp_uno_w1_t0 260320 1057.py
...
#                                erros_locais.append(f"📦 Itens em ESTOQUE detectados: {format_rows(mask_eh_estoque)}")
                            ...
                            mask = (dados_validar != "nan") & (ano_num.isna() | (ano_num < (regra.get("ano_minimo"))) | (ano_num > regra.get("ano_maximo")))
            ...
            if not idx_m.empty:
                ...
                for _, linha in df_dados.iterrows():
                    try:
                        ...
                        contexto_atual = "pedagio" if len(datas) == 1 else "estacto"
                        ...
                        ref_sai_dt = config[f"{contexto_atual}_data"]["sai"]
                        nome_dt = list(ref_sai_dt.keys())[0] if isinstance(ref_sai_dt, dict) else ref_sai_dt

                        registro = {
                            config["COID"]["sai"]: coid,
                            config[f"{contexto_atual}_placa"]["sai"]: str(linha.iloc[idx_placa]).upper(),
                            config[f"{contexto_atual}_data"]["sai"] : data_final,
                            config[f"{contexto_atual}_praca"]["sai"]: local_final,
                            config[f"{contexto_atual}_valor"]["sai"]: valor_final,
                            config[f"{contexto_atual}_tag"]["sai"]  : ""
                        }
