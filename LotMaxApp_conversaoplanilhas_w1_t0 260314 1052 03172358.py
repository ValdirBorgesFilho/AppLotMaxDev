...
arq_saida_tipo = "LV" if executar == "listadeveiculos" else "MP"
...
#                                        dado = df_origem[col_user].astype(str).str.strip()  
                            ...
                            arq_saida=f"{nome_puro}_AppLotMax_{arq_saida_tipo}"            # se executar = listadeveidulos LV ou mapadepneus MP
                                       
                            st.download_button("📥 CSV LotMax para upload", csv_data, f"{arq_saida}_{agora}.csv", "text/csv", use_container_width=True)
                            st.download_button("📄 EXCEL para revisão ", out_xlsx.getvalue(), f"{arq_saida}_revisão_{agora}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
